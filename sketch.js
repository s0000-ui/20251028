...
function drawQuestion() {
  ...
  for (let i = 0; i < q.options.length; i++) {
    ...
    // 自訂背景顏色
    let bg;
    if (selected === i) bg = color(200, 255, 230); // 選中時保持淡綠
    else if (isHover) bg = color(255, 255, 255, 220); // hover 時微白
    else {
      // 每個選項固定顏色
...
function drawResult() {
  ...
  text('得分：' + score + ' / ' + quiz.length, width/2, cy + 110);

  textSize(min(18, width * 0.018));
  let msg = '';
  if (score === quiz.length) msg = '完美！做得很好！';
  else if (score >= floor(quiz.length * 0.7)) msg = '表現不錯，稍加複習可以更好。';
